...
## Considerations 
These are thins we observed but we do not want fixed since future steps will break.
- We have default prompt eg. prompts/system.md its supposed to be scoped on task eg. prompts/
<task>/system.md
- Our settings file should accept .yml or .yaml. right not it only .yaml